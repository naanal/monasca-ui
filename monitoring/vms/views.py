...
from monitoring.vms import constants
...
def get_icon(status):
    ...
    if status == 'PowerOff':
        return constants.LOW_ICON
    if status == 'UnHealthy':
        return constants.MEDIUM_ICON
    if status == 'chicklet-notfound':
        return constants.NOTFOUND_ICON


priorities = [
    {'status': 'Healthy', 'severity': 'OK'},
    {'status': 'Calculating', 'severity': 'UNDETERMINED'},
    {'status': 'PowerOff', 'severity': 'LOW'},
...
def get_dashboard_links(request):
    ...
    try:
        ...
        LOG.warn(
            "Failed to parse dashboard links by project, returning defaults.")
        pass
    #
...
def get_monitoring_services(request):
    ...
    try:
        ...
        LOG.warn(
            "Failed to parse monitoring services by project, returning defaults.")
        pass
    #
    # Extra safety here -- should have got a match somewhere above,
    # but fall back to defaults.
    ...
    for row in monitoring_services:
        ...
        if 'groupBy' in row:
            ...
            for group, group_alarms in alarms_by_group.items():
                ...
                print(group_alarms)
                print("**************")
                for ialarm in group_alarms:
                    alarm = {}
                    alarm['name'] = ialarm['alarm_definition']['name']
                    alarm['metrics']=ialarm['metrics']
                    alarm['status'] = ialarm['state']
                    ialarms.append(alarm)
                service = {
                    'display': group,
        ...
        #alarm_services.append(alarms_by_group)
    print(monitoring_services)
